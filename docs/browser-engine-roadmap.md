...
当前 `/bin/browser` 是 OpenOS 自研轻量浏览器，用于验证 GUI、HTTP/文件加载、HTML 文本化解析、最小 DOM/CSS 分层、历史导航、表单雏形、图片占位、HTTP header 诊断、重定向和滚动阅读能力。

它不是 Chrome 引擎，也不需要伪装成 Chrome 引擎。Chromium 路线保持冻结；恢复条件是依赖闭包、构建体积、许可证、内存占用和维护成本均重新评估通过。

## 近期主线

1. 继续完善 `src/user/browser_engine.h` 的 tokenizer/parser/DOM/style 分层。
2. 扩展默认 CSS display、基础选择器和简单块布局。
3. 巩固 P4/P5/P6：表单雏形、图片占位、HTTP header、重定向、缓存恢复和本地样例回归。
