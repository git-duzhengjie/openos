@echo off
rem  GUI + USB trace + HMP monitor for scripted key injection
rem  [fix] 键盘走 PS/2：PS/2 键盘与 usb-kbd 并存会抢焦点导致锁屏无法输入，故去掉 usb-kbd，仅保留 usb-mouse
set "ROOT=E:\openos"
...
echo [trace] GUI+QMP mode; serial -^> %SERLOG% ; usb trace -^> %TRACELOG% ; monitor tcp 5555
"%QEMU%" -machine pc -cpu qemu64 -smp 4 -m 256M ^
  ...
  -qmp tcp:127.0.0.1:55666,server,nowait ^
  -D "%TRACELOG%" ^
