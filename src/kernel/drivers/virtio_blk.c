...
#define VIRTIO_MODERN_BLK_DEVICE   0x1042u
...
static int virtio_blk_flush(blockdev_t *dev) {
    (void)dev;
    return 0;
}
...
static void virtio_blk_probe_function(uint8_t bus, uint8_t dev, uint8_t func) {
    ...
    strcpy(vdev->name, "vblk");
    suffix[0] = (char)('0' + virtio_blk_count);
    suffix[1] = '\0';
    strcat(vdev->name, suffix);

    if (blockdev_register(vdev->name, 5u, virtio_blk_count,
                          VIRTIO_BLK_SECTOR_SIZE, vdev->sectors,
                          &virtio_blk_ops, vdev, &vdev->blockdev) != 0) {
        serial_write("virtio-blk: failed to register blockdev\n");
        memset(vdev, 0, sizeof(*vdev));
        return;
    }

    (void)vfs_mknod(vdev->name, VFS_MODE_TYPE_BLOCK | 0660u, 5u, virtio_blk_count);
...
void virtio_blk_init(void) {
    ...
    virtio_blk_ops.read = virtio_blk_read_blocks;
    virtio_blk_ops.write = virtio_blk_write_blocks;
    virtio_blk_ops.flush = virtio_blk_flush;
    ...
    serial_write_dec(virtio_blk_count);
    serial_write("\n");
}
