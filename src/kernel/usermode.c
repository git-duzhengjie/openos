/* ============================================================
 * openos - 用户态切换实现
 * 修复: 用户代码/栈必须用 USER 标志映射，否则 Ring 3 无法访问
 * ============================================================ */
...
/* 用户代码页地址（必须是用户可访问的） */
#define USER_CODE_ADDR    0x40000000  /* 1GB 处，专用于用户代码 */
#define USER_CODE_SIZE    4096

/* 用户栈地址 */
#define USER_STACK_ADDR   0xBF000000
...
uint32_t alloc_user_stack(void)
{
    ...
    if (!phys) {
        serial_write("[USERMODE] ERROR: cannot alloc user stack\n");
        return 0;
    }
