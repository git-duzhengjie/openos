...
/* D1-B: AF-pattern trap. If iret would resume to 0xAF... garbage,
 * freeze here and dump context so we catch the writer red-handed. */
#include "../include/early_console64.h"
static void d1b_af_trap(uint64_t iret_rip, uint64_t iret_cs) {
    /* Cheap fingerprint: any byte 0xAF in the top 4 bytes of RIP. */
    uint64_t hi = (iret_rip >> 32) & 0xFFFFFFFFULL;
    if (hi == 0 || hi == 0xFFFFFFFFULL) return; /* kernel/user canonical */
    /* Also catch full-pattern 0xAFAF... */
    uint32_t b0 = (hi >> 0) & 0xFF;
    uint32_t b1 = (hi >> 8) & 0xFF;
    if (b0 != 0xAF && b1 != 0xAF) return;
    early_console64_write("\n[d1-B] AF-PATTERN TRAP rip=0x");
    early_console64_write_hex64(iret_rip);
    early_console64_write(" cs=0x");
    early_console64_write_hex64(iret_cs);
    early_console64_write("\n");
    for (;;) { __asm__ volatile("cli; hlt"); }
}

/* γ.5-D1-J: per-tick ring buffer probe.
 * 目的：每 tick 采样当前 slot + iret_rsp，供 GP 现场回放。
 * 判评：如果 GP 时最新一条 slot_state==FREE/EXITED 或 iret_rsp 不属于 slot 栈区间，
 *       则坐实 stale-ctx / UAF 假说。
 */
#define OPENOS_LAPIC_J_RING_LEN 32u
typedef struct {
    uint64_t  tick_no;
    uint32_t  cpu;
    uint32_t  slot_idx;
...
static uint32_t        g_j_head;
static uint64_t        g_j_tick;
...
static void j_probe_record(uint64_t iret_cs,
    ...
    uint32_t slot = arch_x86_64_sched_current_slot();
    lapic_j_entry_t *e = &g_j_ring[idx];
    e->tick_no         = tno;
    e->cpu             = arch_x86_64_this_cpu_ptr()->cpu_idx;
    e->slot_idx        = slot;
    e->slot_state      = (slot < OPENOS_X86_64_SCHED_MAX_KTHREADS)
                         ? (uint32_t)arch_x86_64_sched_slot_state(slot)
                         : 0xFFFFFFFFu;
    e->slot_kstack_top = (slot < OPENOS_X86_64_SCHED_MAX_KTHREADS)
                         ? arch_x86_64_sched_slot_kstack_top(slot) : 0;
    e->iret_cs         = iret_cs;
    e->iret_rip        = iret_rip;
    e->iret_rsp        = iret_rsp;
    e->iret_ss         = iret_ss;
...
void arch_x86_64_lapic_j_probe_dump(void) {
    early_console64_write("[J-probe] ring dump (last 8, newest first):\n");
    uint32_t head = __atomic_load_n(&g_j_head, __ATOMIC_RELAXED);
    uint32_t n    = (head < 8u) ? head : 8u;
    for (uint32_t i = 0; i < n; ++i) {
        uint32_t idx = (head - 1u - i) % OPENOS_LAPIC_J_RING_LEN;
        lapic_j_entry_t *e = &g_j_ring[idx];
        early_console64_write("  t=");
        early_console64_write_hex64(e->tick_no);
        early_console64_write(" cpu=");
        early_console64_write_hex64((uint64_t)e->cpu);
        early_console64_write(" slot=");
        early_console64_write_hex64((uint64_t)e->slot_idx);
        early_console64_write(" st=");
        early_console64_write_hex64((uint64_t)e->slot_state);
        early_console64_write(" ktop=");
        early_console64_write_hex64((uint64_t)e->slot_kstack_top);
        early_console64_write(" cs=");
        early_console64_write_hex64(e->iret_cs);
