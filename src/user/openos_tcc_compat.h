...
unsigned long long __udivdi3(unsigned long long a, unsigned long long b) { unsigned long long q = 0, r = 0; int i; if (!b) return 0; for (i = 63; i >= 0; i--) { r = (r << 1) | ((a >> i) & 1ULL); if (r >= b) { r -= b; q |= (1ULL << i); } } return q; }
unsigned long long __umoddi3(unsigned long long a, unsigned long long b) { unsigned long long r = 0; int i; if (!b) return 0; for (i = 63; i >= 0; i--) { r = (r << 1) | ((a >> i) & 1ULL); if (r >= b) r -= b; } return r; }
